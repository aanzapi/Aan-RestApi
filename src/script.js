...
function setupUI() {
    ...
    if (titleApi) titleApi.textContent = settings.name || "AanzDigital";
    if (descApi) descApi.textContent = settings.description || "Interactive API documentation with real-time testing";
    if (footer) footer.textContent = `© ${new Date().getFullYear()} ${settings.creator || "AanzDigital"} - ${settings.name || "AanzDigital"}`;
...
async function executeRequest(event, catIndex, endpointIndex, method, path, produces) {
    ...
    try {
        ...
        const response = await fetch(url, {
            ...
            headers: {
                'Accept': '*/*',
                'User-Agent': 'AanzDigital-API-Docs'
            }
        });
